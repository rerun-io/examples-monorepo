...
## Planned: a generated wiki

Long-form package docs are being cut back to short references (see
`packages/slam-rs/docs/`). The plan is to replace them with a wiki generated by
[openwiki](https://github.com/langchain-ai/openwiki) and published with
[zensical](https://zensical.org), once we decide to switch. What has been
worked out so far, so it is not rediscovered:

- One wiki at the repo root in `openwiki/`, packages as sections. openwiki has
  no monorepo mode: the output directory is fixed and the root is the git root.
  `openwiki/INSTRUCTIONS.md` steers the sections; `.openwikiignore` keeps
  `data/`, vendored code and lockfiles out.
- Generation is host-driven: a maintainer's Codex or Claude Code session runs
  the openwiki integration, so no provider key lives in the repo or in CI.
  No scheduler at first; if refreshes get forgotten, openwiki's own Actions
  template can open a weekly PR with an org secret.
- Generated pages are never hand-edited; a correction goes into code or a README
  and the wiki is regenerated. Updates are git-diff based and a no-op costs
  nothing.
- The site: a root `docs/` with a landing page, the wiki copied in at build
  time and git-ignored, package READMEs pulled in as snippets, built with
  `zensical build --strict` and published to GitHub Pages by one workflow that
  calls no model.
- Tooling lives in its own `docs` pixi feature and environment (openwiki is
  already packaged as a conda package on our prefix.dev channel; it needs node
  22, which must not enter every environment through `common`). Telemetry off
  in that feature's activation env. Delete the `.github/workflows/openwiki-update.yml`
  that `openwiki code --init` writes.
- The first `--init` over the whole monorepo is one long agent run with an
  unknown token cost; there is no estimate mode.
