...
camera_counts = st.integers(min_value=1, max_value=4)
...
def test_strictly_increasing_timestamps_are_accepted(start: int, steps: list[int]) -> None:
...
def test_any_non_increasing_timestamp_is_rejected(start: int, steps: list[int], back: int) -> None:
    vio: _core.Vio = _core.Vio(camera_count=1)
...
@settings(max_examples=MAX_EXAMPLES, deadline=None)
@given(start=starts, steps=gaps)
def test_an_increasing_batch_is_accepted(start: int, steps: list[int]) -> None:
    vio: _core.Vio = _core.Vio(camera_count=1)
    times: NDArray[np.int64] = increasing(start, steps)
...
def test_a_batch_that_repeats_one_timestamp_is_rejected(start: int, steps: list[int], index: int) -> None:
...
def test_mismatched_batch_lengths_are_rejected(times: int, rows: int) -> None:
    assume(times != rows)
    vio: _core.Vio = _core.Vio(camera_count=1)
...
@settings(max_examples=MAX_EXAMPLES, deadline=None)
@given(count=camera_counts, height=sizes, width=sizes)
def test_any_contiguous_uint8_frameset_is_accepted(count: int, height: int, width: int) -> None:
    vio: _core.Vio = _core.Vio(camera_count=count)
    image: UInt8[np.ndarray, "h w"] = np.zeros((height, width), dtype=np.uint8)
