...
def test_camera_and_imu_share_one_clock_and_window(clock_recording: Path, settings: SlamConfig, offset_ns: int, cache_video: bool) -> None:
    """Select the sample at the frame's estimator time, excluding both neighbours."""
    parameters: ImuParameters = replace(settings.robocap.imu, cam_time_offset_ns=offset_ns)
...
def test_publishing_and_replacing_estimates_cannot_change_ground_truth(clock_recording: Path, tmp_path: Path, settings: SlamConfig) -> None:
    parameters: ImuParameters = replace(settings.robocap.imu, cam_time_offset_ns=0)
...
def test_unscored_feed_does_not_open_ground_truth_storage(clock_recording: Path, settings: SlamConfig) -> None:
    ...
    with rr.server.Server(datasets={"test": [clock_recording]}) as server:
        ...
        with open_segment(source, settings.robocap.imu, include_ground_truth=False) as feed:
