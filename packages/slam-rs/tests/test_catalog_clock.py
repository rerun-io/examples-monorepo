...
from slam_rs.reference import ImuParameters, ReferenceManifest
from slam_rs.trajectory import Trajectory


@pytest.fixture
def clock_recording(tmp_path: Path) -> Path:
    ...
    with rr.RecordingStream("clock-test", recording_id="clock") as rec:
        ...
        rec.log("/world/rig_00/cam_00", rr.Transform3D(translation=[0, 0, 0], mat3x3=np.eye(3), relation=rr.TransformRelation.ChildFromParent), static=True)
        rec.log(
...
def test_camera_and_imu_share_one_clock_and_window(clock_recording: Path, manifest: ReferenceManifest, offset_ns: int, cache_video: bool) -> None:
    """Select the sample at the frame's estimator time, excluding both neighbours."""
    parameters: ImuParameters = replace(manifest.robocap.imu, cam_time_offset_ns=offset_ns)
    with open_segment(LocalSegment(clock_recording), parameters, cache_video=cache_video) as feed:
...
def test_publishing_and_replacing_estimates_cannot_change_ground_truth(clock_recording: Path, tmp_path: Path, manifest: ReferenceManifest) -> None:
    parameters: ImuParameters = replace(manifest.robocap.imu, cam_time_offset_ns=0)
    with rr.server.Server(datasets={"test": [clock_recording]}) as server:
        ...
        for layer, stamps, positions in [("gt", [10, 30], [1.0, 3.0]), ("slam_rs", [10, 20], [99.0, 88.0]), ("slam_rs", [10, 20], [77.0, 66.0])]:
            ...
            with open_segment(CatalogSegment(server.url(), "test", "clock"), parameters) as feed:
...
def test_unscored_feed_does_not_open_ground_truth_storage(clock_recording: Path, manifest: ReferenceManifest) -> None:
    """Layer generation needs only base cameras/IMU, even if GT storage is server-local."""
    with rr.server.Server(datasets={"test": [clock_recording]}) as server:
        source: CatalogSegment = CatalogSegment(
            server.url(), "test", "clock", dataset=server.client().get_dataset("test"), ground_truth_uri="file:///server-only/gt.rrd",
        )
        with open_segment(source, manifest.robocap.imu, include_ground_truth=False) as feed:
