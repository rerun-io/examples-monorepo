...
/// `Pattern24` (`patterns.h:44-79`), 24 taps.
///
/// ```text
///          00  01
///      02  03  04  05
...
pub struct Pattern24;

/// `patterns.h:60-74`, verbatim.
const PATTERN24_RAW: [[f32; 2]; 24] = [
    [-1.0, 5.0],
    [1.0, 5.0],
    [-3.0, 3.0],
    [-1.0, 3.0],
...
impl Pattern for Pattern24 {
    const SIZE: usize = 24;
    const CODE: i32 = 24;
    const OFFSETS: &'static [[f32; 2]] = &PATTERN24_RAW;
}
...
/// `Pattern50` = `0.75 * Pattern52` (`patterns.h:148-158`).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Pattern50;

/// `0.75 * Pattern52::pattern2` (`patterns.h:158`).
const PATTERN50_RAW: [[f32; 2]; 52] = scaled_pattern52(0.75);

impl Pattern for Pattern50 {
    const SIZE: usize = 52;
    const CODE: i32 = 50;
    const OFFSETS: &'static [[f32; 2]] = &PATTERN50_RAW;
}
...
mod tests {
    ...
    fn every_pattern_fits_the_shared_capacity() {
        const { assert!(Pattern24::SIZE <= MAX_PATTERN_SIZE) };
        assert_eq!(Pattern52::SIZE, MAX_PATTERN_SIZE);
        assert_eq!(Pattern51::SIZE, Pattern52::SIZE);
        assert_eq!(Pattern50::SIZE, Pattern52::SIZE);
        assert_eq!(Pattern24::OFFSETS.len(), Pattern24::SIZE);
        assert_eq!(Pattern52::OFFSETS.len(), Pattern52::SIZE);
    }

    #[test]
    fn pattern51_is_half_of_pattern52_and_pattern50_three_quarters() {
        for i in 0..Pattern52::SIZE {
            for axis in 0..2 {
                ...
                assert_eq!(
                    Pattern50::OFFSETS[i][axis],
                    0.75 * Pattern52::OFFSETS[i][axis]
                );
            }
        }
    }
    ...
    /// Both `Pattern24` and `Pattern52` are symmetric about the origin, which is
    /// what makes the SE(2) rotation column of `Jw_se2` (`patch.h:114-115`)
    /// mean-free before normalisation.
    #[test]
    fn the_patterns_are_symmetric_about_the_origin() {
        for offsets in [Pattern24::OFFSETS, Pattern52::OFFSETS] {
