...
def write_layer(
    ...
    *,
    ...
    root: str = schema.run_path("slam_rs")
    with (
        atomic_write(path) as temporary,
        rr.RecordingStream("dataforge", recording_id=segment_id, send_properties=False) as rec,
    ):
        rec.save(temporary)
        ...
        rec.log(schema.trajectory_path("slam_rs"), rr.LineStrips3D(np.stack([overview[:-1], overview[1:]], axis=1), colors=colors[indices[:-1]], radii=rr.Radius.ui_points(1.5)), static=True)
        ...
        rec.log(schema.trail_path("slam_rs"), rr.LineStrips3D.from_fields(radii=0.006), static=True)
