...
def img_gradient(
    ...
    gx_kernel: Float[Tensor, "c 1 3 3"] = (1.0 / 32.0) * torch.tensor(
    ...
    gy_kernel: Float[Tensor, "c 1 3 3"] = (1.0 / 32.0) * torch.tensor(
    ...
    padded: Float[Tensor, "b c h_pad w_pad"] = F.pad(img, (1, 1, 1, 1), mode="reflect")

    gx: Float[Tensor, "b c h w"] = F.conv2d(padded, gx_kernel, groups=c)
    gy: Float[Tensor, "b c h w"] = F.conv2d(padded, gy_kernel, groups=c)
