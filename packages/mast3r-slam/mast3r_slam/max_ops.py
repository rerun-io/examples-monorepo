...
def _get_custom_ops() -> Any:
    ...
    kernels = Path(__file__).resolve().parent / "backend" / "mojo" / "gn_rays_ops"
...
def preferred_mojo_interface() -> str:
    requested = os.environ.get("MAST3R_SLAM_MOJO_INTERFACE", "auto").strip().lower()
    if requested in {"shared_lib", "shared-lib"}:
        return "shared_lib"
    if requested == "custom_op":
        return "custom_op"
    return "shared_lib"
