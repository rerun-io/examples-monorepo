...
def mast3r_symmetric_inference(
    ...
    if profile is None:
        ...
        with timed_section(profile, "decode_ms", sync_cuda=True):
            res11, res21 = decoder(model, feat1, feat2, pos1, pos2, shape1, shape2)
            res22, res12 = decoder(model, feat2, feat1, pos2, pos1, shape2, shape1)
...
def mast3r_inference_mono(
    ...
    if frame.feat is None:
        if profile is None:
            ...
            with timed_section(profile, "encode_ms", sync_cuda=True):
                frame.feat, frame.pos, _ = model._encode_image(frame.rgb_tensor, frame.img_true_shape)
...
def mast3r_match_symmetric(
    ...
    if profile is not None:
        profile["batch_size"] = int(feat_i.shape[0])
    X, C, D, Q = mast3r_decode_symmetric_batch(model, feat_i, pos_i, feat_j, pos_j, shape_i, shape_j, profile=profile)
    ...
    if profile is None:
        ...
        with timed_section(profile, "matching_ms", sync_cuda=True):
            idx_1_to_2, valid_match_2 = matching.match(X11, X21, D11, D21)
    # toc("Match")
...
def mast3r_match_asymmetric(
    ...
    """
    X, C, D, Q = mast3r_asymmetric_inference(model, frame_i, frame_j, profile=profile)
