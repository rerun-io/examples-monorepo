...
## Rerun Reference Links

These Rerun SDK features are directly relevant to the logging decoupling:

| Link | Summary |
|---|---|
| [Multithreading example](https://rerun.io/examples/feature-showcase/multithreading) | `rr.log()` is thread-safe; no explicit locking needed across threads sharing a recording. Multiple threads can call `rr.log()` simultaneously. |
| [Multiprocess logging](https://rerun.io/examples/feature-showcase/multiprocess_logging) | Each child process needs its own `rr.init()` + `rr.connect_grpc()` + `@rr.shutdown_at_exit`. Data from all processes aggregates in the viewer via gRPC. |
| [Latest-at semantics](https://rerun.io/docs/concepts/logging-and-ingestion/latest-at) | The viewer queries the most recent value of each component at or before the current time. Only log what changes; unchanged components retain their previous values automatically. |
| [send_columns](https://rerun.io/docs/howto/logging-and-ingestion/send-columns) | Batch-send pre-recorded data across multiple timestamps in a single call. More efficient than looping with `rr.log()` for pre-existing datasets. Does NOT add automatic `log_time`/`log_tick` timelines. |
| [Partial updates](https://rerun.io/docs/howto/logging-and-ingestion/send-partial-updates) | Use `Archetype.from_fields()` to log only changed components. Combined with latest-at, this enables delta encoding where different threads/processes update different aspects of the same entity at different rates. |

### Recording Binding for Cross-Thread Logging

The key API for sharing a recording across threads:

```python
# In the Gradio callback thread (after @rr.thread_local_stream):
recording = rr.get_thread_local_data_recording()

# In the logger thread:
rr.set_thread_local_data_recording(recording)
# Now rr.log() in this thread writes to the same recording/binary stream
```

Verified: `rr.log()` and `rr.send_blueprint()` both work cross-thread with this pattern. Bytes logged from the logger thread are visible to `stream.read()` in the Gradio callback thread.
