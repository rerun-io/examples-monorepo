"""Tests for the shared Polycam completion batching preamble."""
...
def test_stack_polycam_batch_returns_cuda_ready_raw_inputs(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cuda_calls: list[torch.Tensor] = []

    def fake_cuda(tensor: torch.Tensor) -> torch.Tensor:
    ...
    tensor_batch = stack_polycam_batch(frames)

    assert tensor_batch.rgb_bhw3.dtype is torch.uint8
    assert tensor_batch.prompt_bhw.dtype is torch.float32
    assert_array_equal(tensor_batch.rgb_bhw3.numpy(), np.stack([frame.rgb_hw3 for frame in frames]))
    assert_array_equal(tensor_batch.prompt_bhw.numpy(), np.stack([frame.original_depth_hw for frame in frames]) / 1000.0)
    assert cuda_calls == [tensor_batch.rgb_bhw3, tensor_batch.prompt_bhw]
