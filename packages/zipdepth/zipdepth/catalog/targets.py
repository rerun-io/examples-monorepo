...
MAX_DEPTH_MM: int = int(MAX_METRIC_DEPTH_M * 1000)
...
def build_train_transform(height: int, width: int, policy: AugmentPolicy) -> AlbumentationsWrapper:
    ...
    color augmentation but resizes deterministically to the training size. RGB
    and target sampling is centre-aligned, so they read the same scene coordinate
    and horizontal flip commutes with target resize.
    """
...
def build_eval_transform(height: int, width: int) -> AlbumentationsWrapper:
    """Build centre-aligned RGB/target resizing that commutes with horizontal flip."""
    if height <= 0 or width <= 0:
        raise ValueError("evaluation height and width must be positive")
    transform: A.ReplayCompose = A.ReplayCompose([], additional_targets={"depth": "mask"})
    return AlbumentationsWrapper(transform, resize_hw=(height, width))
...
def _transform_with_flip_state(
    ...
    """Apply RGB/target augmentation and return the wrapper's explicit flip state."""
    return transform.apply_with_flip(rgb_hwc, target_hw)
...
def _prompt_tensors(
    ...
    *,
    ...
    prompt_depth_chw: Float32[ndarray, "1 192 256"] = np.ascontiguousarray(prompt_depth_m_hw[None])
    prompt_valid_chw: Bool[ndarray, "1 192 256"] = np.ascontiguousarray(prompt_valid_hw[None])
    return {
        "prompt_depth": torch.from_numpy(prompt_depth_chw),
        "prompt_valid": torch.from_numpy(prompt_valid_chw),
    }
...
def depth_span_ratio_cuda(depth_mm_hw: Shaped[Tensor, "h w"]) -> Float32[Tensor, ""]:
    ...
    valid_count: Int64[Tensor, ""] = valid_n.sum()
    last_valid_index: Float32[Tensor, ""] = torch.clamp(valid_count - 1, min=0).to(dtype=torch.float32)

    positions: Float32[Tensor, "2"] = last_valid_index * torch.tensor([0.05, 0.95], device=depth_mm_hw.device)
    lower_float: Float32[Tensor, "2"] = torch.floor(positions)
    lower_indices: Int64[Tensor, "2"] = lower_float.to(dtype=torch.int64)
    upper_indices: Int64[Tensor, "2"] = torch.ceil(positions).to(dtype=torch.int64)
    percentiles: Float32[Tensor, "2"] = torch.lerp(sorted_n[lower_indices], sorted_n[upper_indices], positions - lower_float)
    p5: Float32[Tensor, ""] = percentiles[0]
    p95: Float32[Tensor, ""] = percentiles[1]
...
def build_training_sample(
    ...
    *,
    ...
    Args:
        rgb_hwc: Unsigned 8-bit RGB image with shape ``(H, W, 3)``.
    ...
    transformed: tuple[UInt8[ndarray, "out_h out_w 3"], Float32[ndarray, "out_h out_w"], bool] = _transform_with_flip_state(
        rgb_hwc, inverse_depth_hw, transform
    )
    augmented_rgb_hwc: UInt8[ndarray, "out_h out_w 3"] = transformed[0]
    ...
    image_chw: UInt8[ndarray, "3 out_h out_w"] = np.ascontiguousarray(np.moveaxis(augmented_rgb_hwc, -1, 0))
    depth_chw: UInt16[ndarray, "1 out_h out_w"] = np.ascontiguousarray(quantized_hw[None])
    mask_chw: Bool[ndarray, "1 out_h out_w"] = np.ascontiguousarray(depth_chw > 0)
    image_tensor: UInt8[Tensor, "3 out_h out_w"] = torch.from_numpy(image_chw)
    depth_tensor: UInt16[Tensor, "1 out_h out_w"] = torch.from_numpy(depth_chw)
    mask_tensor: Bool[Tensor, "1 out_h out_w"] = torch.from_numpy(mask_chw)
...
def build_metric_training_sample(
    ...
    Args:
        rgb_hwc: Oriented uint8 RGB with shape ``(H, W, 3)``.
    ...
    """
    # Keep every real teacher value (0 mm is the invalid encoding); target_valid
    # carries the fixed metric training and evaluation window.
    ...
    transformed: tuple[UInt8[ndarray, "out_h out_w 3"], Float32[ndarray, "out_h out_w"], bool] = _transform_with_flip_state(
        rgb_hwc, target_depth_m_hw, transform
    )
    augmented_rgb_hwc: UInt8[ndarray, "out_h out_w 3"] = transformed[0]
    augmented_target_depth_hw: Float32[ndarray, "out_h out_w"] = transformed[1]
    image_chw: UInt8[ndarray, "3 out_h out_w"] = np.ascontiguousarray(np.moveaxis(augmented_rgb_hwc, -1, 0))
    target_depth_chw: Float32[ndarray, "1 out_h out_w"] = np.ascontiguousarray(augmented_target_depth_hw[None])
    target_valid_chw: Bool[ndarray, "1 out_h out_w"] = np.ascontiguousarray(
        (target_depth_chw >= MIN_DEPTH_MM / 1000.0) & (target_depth_chw <= MAX_DEPTH_MM / 1000.0)
    )
...
def build_training_sample_cuda(
    ...
    *,
    ...
    resizing matches the CPU path's antialiased RGB and exact nearest-neighbor
    depth policy. Both paths sample centre-aligned coordinates, so RGB and target
    stay aligned and horizontal flip commutes with target resize. The returned
    depth uses uint16 storage, but all arithmetic is performed in float32 or
    int32 because PyTorch has limited uint16 support.
    ...
    depth_working_mm_hw: Float32[Tensor, "rotated_h rotated_w"] = rotated_depth_mm_hw.to(dtype=torch.float32)
    if target_mode not in ("ssi", "metric"):
        raise ValueError(f"unknown target mode {target_mode!r}")
    if (prompt_depth_mm_hw is None) != (prompt_confidence_hw is None):
        raise ValueError("prompt depth and confidence must be provided together")
    if target_mode == "ssi":
        valid_hw: Bool[Tensor, "rotated_h rotated_w"] = (depth_working_mm_hw >= MIN_DEPTH_MM) & (depth_working_mm_hw <= MAX_DEPTH_MM)
        target_hw: Float32[Tensor, "rotated_h rotated_w"] = torch.where(
            valid_hw,
            1000.0 / depth_working_mm_hw,
            torch.zeros_like(depth_working_mm_hw),
        )
    else:
        # Keep every real teacher value (0 mm is the invalid encoding); target_valid
        # carries the fixed metric training and evaluation window.
        target_hw = torch.where(
            depth_working_mm_hw > 0,
            depth_working_mm_hw / 1000.0,
            torch.zeros_like(depth_working_mm_hw),
        )
    resized: tuple[UInt8[Tensor, "3 out_h out_w"], Float32[Tensor, "out_h out_w"] | None] = resize_rgb_and_depth_exact(
        rotated_rgb_chw, target_hw, (out_height, out_width)
    )
    resized_rgb_chw: UInt8[Tensor, "3 out_h out_w"] = resized[0]
    resized_target_hw: Float32[Tensor, "out_h out_w"] | None = resized[1]
    if resized_target_hw is None:
    ...
    if random_values[2] < policy.gray_p:
        rgb_for_gray_chw: Float32[Tensor, "3 out_h out_w"] = resized_rgb_chw.to(dtype=torch.float32)
    ...
    if target_mode == "ssi":
        quantized_hw: Shaped[Tensor, "out_h out_w"] = torch.round(resized_target_hw * INV_DEPTH_SCALE).clamp_(0.0, 65535.0).to(dtype=torch.int32)
        mask_chw: Bool[Tensor, "1 out_h out_w"] = (quantized_hw > 0).unsqueeze(0).contiguous()
        depth_chw: UInt16[Tensor, "1 out_h out_w"] = quantized_hw.to(dtype=torch.uint16).unsqueeze(0).contiguous()
        sample: dict[str, Tensor] = {"image": image_chw, "depth": depth_chw, "mask": mask_chw}
    else:
        target_depth_chw: Float32[Tensor, "1 out_h out_w"] = resized_target_hw.unsqueeze(0).contiguous()
        target_valid_chw: Bool[Tensor, "1 out_h out_w"] = (
            (target_depth_chw >= MIN_DEPTH_MM / 1000.0) & (target_depth_chw <= MAX_DEPTH_MM / 1000.0)
        )
        sample = {
            "image": image_chw,
            "target_depth": target_depth_chw,
            "target_valid": target_valid_chw,
        }
    if prompt_depth_m_hw is not None and prompt_valid_hw is not None:
