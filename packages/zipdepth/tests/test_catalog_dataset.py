...
import pytest
import torch
from jaxtyping import Bool, UInt8, UInt16
...
from zipdepth.catalog.dataset import CatalogPromptDepthDataset, ShuffleBuffer, _ProducerSlot, component_bytes_views, promptda_blob_views  # noqa: E402
from zipdepth.catalog.segments import SegmentRow  # noqa: E402
...
def test_catalog_dataset_constructs_without_contacting_server() -> None:
    ...
    row_by_id: dict[str, SegmentRow] = {
        "segment-a": SegmentRow(
            id="segment-a",
            orientation="portrait",
            orientation_quarter_turns_ccw=1,
            layer_names=("promptda",),
        )
    }
...
def test_cpu_sample_builder_decodes_and_reports_local_stats() -> None:
    ...
    blob_bytes: UInt8[ndarray, "n"] = np.frombuffer(encode_depth_png(depth_mm_hw), dtype=np.uint8)
    ...
    confidence: UInt8[ndarray, "confidence_n"] = confidence_hw.reshape(-1)
...
def test_cpu_metric_builder_orients_prompt_and_confidence_with_rgb_and_target() -> None:
    ...
    expected_valid_hw: Bool[ndarray, "192 256"] = np.rot90(
        (confidence_stored_hw >= 1) & (prompt_stored_mm_hw >= 100) & (prompt_stored_mm_hw <= 4000), 1
    )
...
def test_component_bytes_views_handles_prompt_and_confidence_columns() -> None:
    ...
    views: list[ndarray] = component_bytes_views(column, "/confidence:SegmentationImage:buffer")
