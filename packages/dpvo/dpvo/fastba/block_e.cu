/*
 * block_e.cu -- Efficient E-block GPU kernels for global bundle adjustment.
 *
 * Part of DPV-SLAM (Deep Patch Visual SLAM, Lipson et al., 2024).
 *
 * In Schur complement BA, the normal equations are:
 *
 *     [ B   E ] [dX]   [v]
 *     [ Eᵀ  C ] [dZ] = [u]
 *
 * where B is the pose-pose Hessian, E is the pose-depth cross-term,
 * C is the depth-depth diagonal, and dX/dZ are pose/depth updates.
 *
 * The Schur complement eliminates depths:
 *
 *     S = B - E · Q · Eᵀ,    Q = diag(1/(C + λ))
 *     y = v - E · Q · u
 *     dX = S⁻¹ · y
 *     dZ = Q · (u - Eᵀ · dX)
 *
 * For global BA with many patches (M >> 1000), materializing the dense
 * E matrix (6N × M) is too expensive.  Instead, this file provides an
 * "efficient E-block" implementation that stores E in a compressed
 * per-(frame, patch) lookup table and computes the three required
 * matrix operations via GPU kernels.
 */
...
/* -------------------------------------------------------------------------
 * EEt kernel: computes E · Q · Eᵀ via atomicAdd
 *
 * Each thread processes one (patch, edge-pair) combination.
 * The result is a 6N × 6N symmetric matrix.
 * ---------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
 * Ev kernel: computes E · v  (project depth-space vector to pose-space)
 * ---------------------------------------------------------------------- */
...
/* -------------------------------------------------------------------------
 * Etv kernel: computes Eᵀ · v  (project pose-space vector to depth-space)
 * ---------------------------------------------------------------------- */
