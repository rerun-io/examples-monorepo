...
- The package still builds its own CUDA extensions (`dpvo._cuda_corr` and `dpvo._cuda_ba`) locally through the `_build-dpvo-cuda-kernels` task, which runs automatically as a `depends-on` step of `dpvo-demo`, `dpvo-app`, and `eval-euroc`. You can also build them directly with `pixi run -e dpvo --frozen _build-dpvo-cuda-kernels`.
