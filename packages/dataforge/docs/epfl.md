...
Stack note: `exoego/2-epfl-a` ships five of these layers (`base`, `hand_pose`, `body_pose`, `projections`, `actions`); `hand_mesh`, `body_mesh`, `epfl_mesh.py`, `--smpl-model-root` and the `smplx` dependency land in the stacked `exoego/2-epfl-b`. The mesh paragraphs below describe that part.
