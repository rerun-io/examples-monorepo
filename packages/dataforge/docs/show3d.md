...
present; confidence-zero records may still have joint angles. Hand JSON and
profiles use partial pyserde schemas with unknown fields allowed. Profiles are stored without reserializing them.
...
The prototype's `keyboard_toss-away_83ef` object track sits behind the headset
on every frame despite confidence about 0.50–0.63, while hand projections
align. The later object layer needs a geometric sanity check beyond confidence.

BOP HOT3D IDs differ from the other HOT3D numbering. Match by object name in
`models_info.json`: keyboard is BOP 28, birdhouse_toy is BOP 26. Later mesh
work must strip `KHR_texture_transform` and handle aliases without assets.
No meshes are fetched or logged by BASE.
