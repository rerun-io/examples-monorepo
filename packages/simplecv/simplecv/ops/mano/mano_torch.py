...
class ManoSimpleLayer(Module):
    def __init__(
        ...
        # Create tensors with explicit jaxtyping annotations first, then register as buffers
        th_shapedirs: Float[Tensor, "n_verts=778 dim=3 n_betas=10"] = torch.tensor(
            mano_data.shapedirs, dtype=torch.float32
        )  # (778,3,10)
        self.register_buffer("th_shapedirs", th_shapedirs, persistent=True)

        th_posedirs: Float[Tensor, "n_verts=778 dim=3 n_pose_dims=135"] = torch.tensor(
            mano_data.posedirs, dtype=torch.float32
        )  # (778,3,135)
        self.register_buffer("th_posedirs", th_posedirs, persistent=True)

        th_v_template: Float[Tensor, "1 n_verts=778 dim=3"] = torch.tensor(
            mano_data.v_template, dtype=torch.float32
        ).unsqueeze(0)  # (1,778,3)
        self.register_buffer("th_v_template", th_v_template, persistent=True)

        th_J_regressor: Float[Tensor, "n_joints=16 n_verts=778"] = torch.tensor(
            mano_data.J_regressor, dtype=torch.float32
        )  # (16,778)
        self.register_buffer("th_J_regressor", th_J_regressor, persistent=True)

        th_weights: Float[Tensor, "n_verts=778 n_joints=16"] = torch.tensor(
            mano_data.weights, dtype=torch.float32
        )  # (778,16)
        self.register_buffer("th_weights", th_weights, persistent=True)

        th_faces: Int64[Tensor, "n_faces=1538 3"] = torch.tensor(
            mano_data.f.astype(np.int32), dtype=torch.int64
        )  # (1538,3)
        self.register_buffer("th_faces", th_faces, persistent=True)
        # Convenience alias for vertex count (778)
        self.n_verts: int = self.th_v_template.shape[1]

        # Get hand mean & PCA components
        th_hands_mean: Float[Tensor, "1 45"] = torch.tensor(mano_data.hands_mean, dtype=torch.float32).unsqueeze(
            0
        )  # (1,45)
        self.register_buffer("th_hands_mean", th_hands_mean, persistent=True)

        th_comps: Float[Tensor, "45 45"] = torch.tensor(hands_components, dtype=torch.float32)  # (45,45)
        self.register_buffer("th_comps", th_comps, persistent=True)
...
class MANOLayerTorch(Module):
    ...
    def __init__(
        ...
        if mano_root_dir is None:
            # Resolve repo root as two parents above this file (simplecv/ops/.. -> repo root)
            ...
            if not dest_pkl_path.exists():
                try:
                    # Download from the WiLor-Nano repo assets on Hugging Face Hub
                    # Store under data/pretrained_models/mano_clean/, then copy to data/MANO_*.pkl
                    ...
                    # Ensure flat copy at data/MANO_*.pkl for simple lookup
                    if downloaded_path != dest_pkl_path:
                        shutil.copy2(downloaded_path, dest_pkl_path)
                except Exception as e:  # pragma: no cover - network errors or HF unavailable
                    raise RuntimeError(
                        "Failed to download MANO model. Provide 'mano_root_dir' manually or "
                        "ensure network access. Original error: " + str(e)
                    ) from e

            # Use the data directory as MANO root so ManoSimpleLayer reads data/MANO_*.pkl
        ...
        b: Float32[Tensor, "b=1 10"] = torch.from_numpy(rearrange(betas, "b -> 1 b")).float()
        ...
        v_template: Float32[Tensor, "b=1 n_verts=778 dim=3"] = self._mano_layer.th_v_template
        ...
        j_regressor: Float32[Tensor, "16 n_verts=778"] = self._mano_layer.th_J_regressor
