...
VIEW_ROOT: str = "/"
# This file lives at <pkg>/gsplat_rust_renderer/apis/, so the package root (where
# examples/ sits) is parents[2].
DEFAULT_PLY: Path = Path(__file__).resolve().parents[2] / "examples" / "chair.ply"
...
def splat_blueprint(entity_path: str, gaussians: Gaussians3D) -> rrb.Blueprint:
    ...
    """
    bounds_min: Float32[ndarray, "3"] = gaussians.centers.min(axis=0)
    bounds_max: Float32[ndarray, "3"] = gaussians.centers.max(axis=0)
