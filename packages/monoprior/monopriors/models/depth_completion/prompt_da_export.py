...
"""ARKit LiDAR prompt-depth resolution used during PromptDA training."""
...
class _FusionBreakerOutputs(nn.Module):
    ...
    def forward(self, image: Tensor, prompt_depth: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        ...
        depth_b1hw: Tensor = self.model(image, prompt_depth)
        min_val_b111: Tensor = prompt_depth.amin(dim=(1, 2, 3), keepdim=True)
        max_val_b111: Tensor = prompt_depth.amax(dim=(1, 2, 3), keepdim=True)
        return depth_b1hw, min_val_b111, max_val_b111
...
def export_promptda_onnx(
    ...
    dummy_image_b3hw: Tensor = torch.zeros((2, 3, height, width), dtype=torch.float32, device="cuda")
    dummy_prompt_b1hw: Tensor = torch.rand((2, 1, *PROMPT_DEPTH_HW), dtype=torch.float32, device="cuda") + 0.5
    export_onnx(
        wrapper,
