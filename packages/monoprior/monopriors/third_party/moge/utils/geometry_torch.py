...
from monopriors.third_party.moge.utils.geometry_numpy import solve_optimal_focal_shift, solve_optimal_shift
...
def recover_focal_shift(
    ...
    for batch_index in range(points_bhw3.shape[0]):
        ...
        if focal_b is None:
            ...
            optimal_shift_i, optimal_focal_i = solve_optimal_focal_shift(uv_lr_i_np, points_lr_i_np)
            optimal_focals.append(float(optimal_focal_i))
        else:
            assert focal_np is not None
            optimal_shift_i = solve_optimal_shift(uv_lr_i_np, points_lr_i_np, float(focal_np[batch_index]))
