...
import torch
from .rotation import quat_to_mat, mat_to_quat, quat_to_quaty, quaty_to_quat
...
def extri_intri_to_pose_encoding(
    ...
    Returns:
        torch.Tensor: Encoded camera pose parameters with shape BxSx9.
            ...
            - [7:] = field of view (2D)
            For "absT_quaRy_noFoV" type, the 5 dimensions are:
            - [:3] = absolute translation vector T (3D)
            - [3:5] = rotation as y-axis quaternion quat (2D)
            For "noT_quaR_FoV" type, the 6 dimensions are:
            - [:4] = rotation as quaternion quat (4D)
            - [4:] = field of view (2D)
    """
    # extrinsics: BxSx3x4
    # intrinsics: BxSx3x3
    ...
    if pose_encoding_type == "absT_quaR_FoV":
        ...
        quat_ = mat_to_quat(R)
        quat_y = quat_to_quaty(quat_)
        pose_encoding = torch.cat([T, quat_y], dim=-1).float()
        ...
        fov_h = 2 * torch.atan((H / 2) / intrinsics[..., 1, 1])
        fov_w = 2 * torch.atan((W / 2) / intrinsics[..., 0, 0])
        pose_encoding = torch.cat([quat, fov_h[..., None], fov_w[..., None]], dim=-1).float()
...
def pose_encoding_to_extri_intri(
    ...
    if pose_encoding_type == "absT_quaR_FoV":
        ...
        T = pose_encoding[..., :3]
        quat_y = pose_encoding[..., 3:5]

        quat_ = quaty_to_quat(quat_y)
        R = quat_to_mat(quat_)
        extrinsics = torch.cat([R, T[..., None]], dim=-1)
        ...
        fov_h = pose_encoding[..., 4]
        fov_w = pose_encoding[..., 5]

        R = quat_to_mat(quat)
        extrinsics = torch.cat([R, torch.zeros_like(R[..., :3, :1])], dim=-1)
