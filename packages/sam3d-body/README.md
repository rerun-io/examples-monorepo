...
```bash
pixi run -e sam3d-body-dev python tool/new_gradio_sam3.py
```
...
```bash
# HoCap dataset
pixi run -e sam3d-body-dev python tool/demo_mv_body.py hocap --root-directory data/sample

# RRD file (from ExoEgo pipeline)
pixi run -e sam3d-body-dev python tool/demo_mv_body.py rrd --rrd-path path/to/episode.rrd
```
