...
class RrdCompareConfig:
    """Configuration for comparing a candidate RRD against a reference RRD."""

    reference_rrd: Path = Path("tests/reference_data/wilor_video_30.rrd")
    """Known-good RRD to compare against."""
    candidate_rrd: Path = Path("/tmp/wilor_candidate.rrd")
...
def comparable_schema_columns(schema: pa.Schema, *, index: str) -> tuple[str, ...]:
...
def skipped_schema_columns(schema: pa.Schema, *, index: str) -> tuple[str, ...]:
    """Return sorted schema columns not included in the mechanical comparison."""
    comparable_columns: set[str] = set(comparable_schema_columns(schema, index=index))
...
def _column_values(table: pa.Table, column_name: str) -> list[Any]:
    ...
    return column.combine_chunks().to_pylist()


def _null_mask(table: pa.Table, column_name: str) -> list[bool]:
    column: pa.ChunkedArray = table.column(column_name)
    return column.combine_chunks().is_null().to_pylist()
...
def _query_table(dataset: Any, *, index: str, columns: tuple[str, ...]) -> pa.Table:
    ...
    projection: tuple[str, ...] = (index, *columns)
...
def compare_rrd_reference(config: RrdCompareConfig) -> RrdCompareReport:
    """Compare a candidate RRD against a reference RRD.
    ...
    with rr.server.Server(datasets={"reference": [str(reference_path)], "candidate": [str(candidate_path)]}) as server:
        ...
        reference_view: Any = reference_dataset.filter_contents(["/**"])
        candidate_view: Any = candidate_dataset.filter_contents(["/**"])
        reference_schema: pa.Schema = reference_view.arrow_schema()
        candidate_schema: pa.Schema = candidate_view.arrow_schema()
        reference_columns: tuple[str, ...] = comparable_schema_columns(reference_schema, index=config.index)
        candidate_columns: tuple[str, ...] = comparable_schema_columns(candidate_schema, index=config.index)
        ...
        skipped_columns: tuple[str, ...] = skipped_schema_columns(reference_schema, index=config.index)
